...
export function createClonableTestUndoRedoStacks(
	events: Listenable<TreeViewEvents | CheckoutEvents>,
): {
	...
	const unsubscribeFromCommitApplied = events.on(
		"commitApplied",
		onNewCommit as (commit: CommitMetadata, getRevertible?: RevertibleFactory) => void,
	);
	const unsubscribe = (): void => {
		unsubscribeFromCommitApplied();
		for (const revertible of undoStack) {
